...
def get_all_dataset_metadata_from_openml(
    save_filename="all_dataset_metadata.pkl",
) -> Union[List, List]:
...
def create_metadata_dataframe(all_data_descriptions, data_id, all_datasets):
    descriptions = [
        all_data_descriptions[i].description for i in range(len(all_data_descriptions))
    ]

    all_data_description = dict(zip(data_id, descriptions))

    return pd.DataFrame(
        list(all_data_description.items()), columns=["did", "description"]
    ), all_datasets

...
def load_document_and_create_vector_store(
    ...
    if type_of_data == "dataset":
        ...
        if os.path.exists(persist_directory) and recreate_chroma == False:
            db = Chroma(persist_directory=persist_directory, embedding_function=embeddings, collection_name="datasets")
            return db
        else:
            # load data
            ...
            db = Chroma(embedding_function=embeddings, persist_directory=persist_directory, collection_name="datasets")
        ...
        # if the directory already exists, load the vector store else create a new one
        if os.path.exists(persist_directory) and recreate_chroma == False:
            db = Chroma(persist_directory=persist_directory, embedding_function=embeddings, collection_name="flows")
            return db
        else:
            # load data
            loader = DataFrameLoader(metadata_df, page_content_column="Combined_information")
            ...
            db = Chroma(embedding_function=embeddings, persist_directory=persist_directory, collection_name="flows")
            # add documents to the vector store in batches of 100
            if len(unique_docs) < 200:
                db.add_documents(unique_docs, ids=unique_ids)
            else:
